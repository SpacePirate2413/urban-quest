...
- [ ] **C1.** Source 8 LibriVox voice samples for the AI narrator catalog and upload to Chatterbox. Personality-to-reader suggestions and the upload command are in `apps/creator-station/src/store/useWriterStore.js` (NARRATOR_VOICES catalog) plus the AI-narrate redesign Done entry below. Aim for 15–30s of clean single-speaker English per voice. Filenames must be `marcus.mp3` / `elena.mp3` / `jake.mp3` / `lily.mp3` / `arthur.mp3` / `margaret.mp3` / `draven.mp3` / `vex.mp3` (lowercase, matches the IDs the modal looks up). After upload via `curl -X POST http://localhost:4123/v1/voices -F "voice_file=@marcus.mp3" -F "name=marcus"`, reopen the modal — each voice flips from "Coming soon" to a clickable card with a preview button.
