...
- [ ] **F0.** **Server-side push notification pipeline.** Mobile now installs `expo-notifications` and asks for OS permission, so iOS lists Urban Quest under Settings → Notifications and the user's per-category preferences are saved to AsyncStorage. **Nothing actually fires those notifications yet.** Scope: (a) capture each device's Expo push token via `Notifications.getExpoPushTokenAsync()` after permission is granted, store on a new `User.expoPushTokens` (or related) table; (b) backend service `apps/api/src/lib/push.ts` that posts to Expo's `https://exp.host/--/api/v2/push/send`; (c) emit pushes from existing event hooks (quest approved, near-me match) gated on the user's `notification_preferences_v1` AsyncStorage value (which we'd start mirroring server-side); (d) handle delivery receipts so we know when tokens go invalid. The first push category to wire is `questApproved` — the WebSocket already fires that event in-app, push would mirror it for users who aren't currently in the app.
- [ ] **F1.** **Follower system + "new follower" notification.** Lets players follow creators they like, then notifies them in-app + via push when someone new follows them. Scope: (a) Prisma `Follow` table (followerId, followedId, createdAt with the usual indexes); (b) `POST /users/:id/follow` + `DELETE /users/:id/follow` + `GET /me/followers` API endpoints; (c) Follow / Unfollow button on the creator profile in mobile + creator-station; (d) follower-count surfaced on profile; (e) push notification fired when a new follower lands, gated on the user's `newFollower` notification preference. Re-add the toggle to `apps/mobile/app/profile/notifications.tsx` once the system exists. **Removed from the v1 notification settings UI on 2026-04-29.**
- [ ] **F2.** **Weekly digest email + push.** Personalized weekly summary covering new quests near the player, leaderboard movement, and creator news (e.g. quests their followed creators published). Scope: (a) a daily/weekly background worker that aggregates the events for each user (BullMQ + cron); (b) email send via SendGrid or Resend (creator-station-side, not mobile-side); (c) optional in-app push on Sundays at a user-configurable time; (d) opt-in toggle in `apps/mobile/app/profile/notifications.tsx`. Aligns with the "habitual return" engagement loop common in social/creator apps. **Removed from the v1 notification settings UI on 2026-04-29.**
- [ ] **F3.** **Photo / video / audio capture on scouted waypoints.** Originally a creator could attach media to a Scout-tab waypoint (camera, photo library, or microphone) and the saved attachments would surface in the creator-station's saved-waypoints panel for reuse in scenes. **Removed from the mobile UI on 2026-04-29** because the feature wasn't on the v1 critical path and added App Privacy / permissions surface area we didn't need yet (camera + microphone + photo-library prompts). What's still in place for re-enablement: (a) Prisma columns `ScoutedWaypoint.photos`, `.videos`, `.audioRecordings` on the model; (b) backend `POST /scouted-waypoints/:id/upload` (multipart) endpoint, untouched; (c) `apps/api/uploads/` storage path and the URL shape the mobile app already expected; (d) the API still returns the (empty) arrays in `GET /scouted-waypoints`. What was stripped: media buttons + capture handlers in `apps/mobile/app/(tabs)/write.tsx` (`AddWaypointModal` no longer imports `expo-av` or `expo-image-picker`; `WaypointCard` no longer shows photo thumbs or 📷/🎥/🎤 counts), and the photo/video/audio rendering blocks + `mediaPreview` modal in `apps/creator-station/src/pages/write/WaypointEditor.jsx`. To re-enable: restore the modal's media buttons, re-add the upload loop in `handleSaveWaypoint`, and put back the saved-waypoint card body. **No DB migration needed.**
...
- [ ] **C4.** **Apple Sign In on the web (creator-station).** Optional. Currently the web app only offers Google + email sign-in. Most web users will pick Google or email, but a unified "use the same Apple ID across mobile and web" experience requires this.
  **External setup** (depends on C3 already being done):
  1. In Apple Developer → Certificates, Identifiers & Profiles → Identifiers, take the Services ID from C3 (e.g. `com.urbanquest.app.signin`) and configure its **Web Authentication** section: register the creator-station origin (e.g. `https://urbanquestapp.com`) under "Domains" and the OAuth callback URL (e.g. `https://api.urbanquestapp.com/api/users/auth/apple/callback`) under "Return URLs".
  2. Verify the domain via Apple's `apple-developer-domain-association.txt` file at the configured origin (Apple gives you the file content; serve it from the marketing site).
  **Code work** (≈3 hours):
  1. Backend (`apps/api/src/features/users/auth/apple.handler.ts`) — the `appleAuthHandler` and `appleCallbackHandler` already exist. Verify `appleCallbackHandler` correctly handles Apple's `form_post` response_mode (Apple POSTs the callback rather than GET-ing it), then hand the resulting JWT back via the same fragment-redirect pattern Google uses (`<creator-station-origin>/auth/callback#token=<jwt>`).
  2. Frontend — add a "Sign in with Apple" anchor link on `LoginScreen` in `apps/creator-station/src/App.jsx` next to the Google button, pointing at `${apiBase}/users/auth/apple`. The existing `AuthCallback` route already handles the fragment hand-off.
  3. Style the button per Apple's HIG (use the Apple JS SDK's `<div id="appleid-signin">` if you want Apple's pixel-perfect button rendering; otherwise a custom-styled anchor is acceptable per their guidelines as long as it includes the Apple logo and standard wording).
  4. **Anti-steering note:** mobile users who signed up via Apple already have an Apple-linked account. The web Apple button must look up the same `(provider='APPLE', providerId=<sub>)` pair so they land on the same backend account; this happens automatically via `usersService.findOrCreate` since it's the same code path the mobile flow uses.
...
- [x] **Quest playback rebuild** (2026-04-29) — End-to-end real gameplay flow on mobile. Backend filters quests missing required data out of `GET /quests/public` (helper `isQuestPlayable` + `END_SCENE_ID = '__END__'` constant in `apps/api/src/features/quests/quests.service.ts`). The choice-destination dropdown in the creator-station Create tab lists **scenes** (not waypoints) so a creator can branch to a specific scene even when two scenes share a physical waypoint — supports the "revisit a location with different content" pattern. Mobile `apps/mobile/app/quest/play.tsx` rewritten: real `react-native-maps` map showing the player's blue dot + destination pin, real `expo-location` foreground tracking with **25m arrival radius** (up from 15m to absorb GPS jitter), real `expo-av` Audio/Video playback for `scene.mediaUrl`, branching driven by `choice.sceneId`, AppState listener that shows a "Welcome back — checking your location…" splash on re-foreground, and a `__DEV__`-only **▶ Simulate Arrival** button so the iOS simulator doesn't need GPX files. Completion screen now actually **submits the review** via `api.submitReview` (1–5 stars + optional comment + Skip). "Open in Maps" button preserved for turn-by-turn assist. **Legacy data:** old choices stored as `{text, waypointId}` are migrated transparently on read — the mobile parser routes them to the lowest-orderIndex scene at that waypoint, and the creator-station's destination dropdown displays them at their migrated value the next time the choice is edited and saved. **Testing notes:** in dev, tap Simulate Arrival to fast-forward; for App Store reviewer, set Simulator → Features → Location → Custom Location to a waypoint coord. Android emulator needs a Google Maps API key wired into `app.json` before the map renders — tracked separately as a follow-up if/when we target Android.
